...
function setup_env() {
    ...
    if [ ! -e src/bootstrap.php ]; then
        >&2 echo
        >&2 echo "E: Can't find srv/bootstrap.php. Make sure you run this from repo root."
        >&2 echo
        exit 33
    fi

    php src/bootstrap.php compile
}
...
function place_files() {
    ...
    if [ "$pkgname" == "php-philer" ]; then
        cp -R "$builddir/philer" "$targdir/usr/bin/"
    else
        >&2 echo
